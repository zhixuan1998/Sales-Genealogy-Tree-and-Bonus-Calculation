...
interface SalesNodeTree extends SalesNode {
    isEligible?: boolean;
    vnTotalAmount?: number;
    bonusAmount?: number;
    children: SalesNodeTree[];
}

const salesData: SalesNode[] = [
...
class SalesNodeClass implements SalesNodeTree {
    name: string;
    totalSales: number;
    genPath: string;
    isEligible?: boolean;
    ...
    get parentGenPath(): string {
        return SalesNodeClass.getParentGenPath(this.genPath);
    }

    static getParentGenPath(genPath: string): string {
        const genPathArr = genPath.split(".");

        const parentGenPath =
            genPathArr.length > 1 ? genPathArr.slice(0, -1).join(".") : '';

        return parentGenPath;
    }
}

function arrangeSalesData(salesData: any[]) {
...
function getParentGenPath(genPath: string): string | null {
    const genPathArr = genPath.split(".");

    const parentGenPath =
        genPathArr.length > 1 ? genPathArr.slice(0, -1).join(".") : null;

    return parentGenPath;
}

function getChildrenNode(salesData: SalesNode[], parentNode: SalesNode): SalesNodeClass[] {
...
function generateGenealogyTree(salesData: SalesNode[]): SalesNodeClass {
    const salesDataCopy = salesData.slice();
    const rootNode = salesDataCopy.splice(0, 1)[0] as SalesNodeTree;
...
function bonusCalculation(tree: SalesNodeTree) {
    const directChild = tree.children as SalesNodeTree[];
    const grandChild = directChild.flatMap((node: SalesNodeTree) => node.children);

    const directChildTotalSales = directChild.reduce<number>((acc, node) => (node.isEligible === true ? acc + node.totalSales : acc), 0);
    const grandChildTotalSales = grandChild.reduce<number>((acc, node) => (node.isEligible === true ? acc + node.totalSales : acc), 0);
    const directVnTotalAmount = directChild.reduce<number>((acc, node) => acc + (node.vnTotalAmount ?? 0), 0);
