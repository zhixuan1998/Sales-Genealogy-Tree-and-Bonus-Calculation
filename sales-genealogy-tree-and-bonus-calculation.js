...
function arrangeSalesData(salesData) {
    for (let i = salesData.length - 1; i >= 0; i--) {
        ...
        const parentGenPath = SalesNode.getParentGenPath(currNode.genPath);

        //  if total sales not enough, take from children or pass up to parent
        if (currNode.totalSales < targetAmount) {
            const childrenNode = salesData.filter(node => currNode.genPath === SalesNode.getParentGenPath(node.genPath));
...
class SalesNode {
    ...
    get parentGenPath() {
        return SalesNode.getParentGenPath(this.genPath);
    }

    static getParentGenPath(genPath) {
        const genPathArr = genPath.split('.');

        const parentGenPath = genPathArr.length > 1
            ? genPathArr.slice(0, -1).join('.')
            : null;
...
function getChildrenNode(salesData, parentNode) {
    const childrenNode = salesData.filter(node => parentNode.genPath === SalesNode.getParentGenPath(node.genPath));

    return childrenNode.length
        ? childrenNode.map(node => new SalesNode({ ...node, children: getChildrenNode(salesData, node) }))
        : [];
}
...
function generateGenealogyTree(salesData) {
    ...
    const tree = new SalesNode({
        ...rootNode,
        children: getChildrenNode(salesDataCopy, rootNode)
    });
...
function bonusCalculation(tree) {
    ...
    const directVnTotalAmount = directChild.reduce((acc, node) => acc + node.vnTotalAmount, 0);
